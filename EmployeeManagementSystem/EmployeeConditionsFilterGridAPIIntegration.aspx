<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="EmployeeConditionsFilterGridAPIIntegration.aspx.cs" Inherits="EmployeeManagementSystem.EmployeeConditionsFilterGridAPIIntegration" %>
...
<body class="p-4">
    ...
    <script>
        ...
        function fetchDataFromApi() {
            $.ajax({
                url: 'http://localhost:53393/api/employees/all', // 🔁 Replace port if needed
                method: 'GET',
                success: function (response) {
                    ...
                    //data = response.map(e => ({
                    //    ...e,
                    //    joiningDate: e.joiningDate && !isNaN(new Date(e.joiningDate))
        ...
        function buildTableHeader() {
            ...
            <th class="sortable" data-key="${col.key}">
                ...
                <div class="input-group input-group-sm mt-1">
                    <input type="${isDate ? 'date' : 'text'}" class="form-control filter-input" data-key="${col.key}" placeholder="Filter..." />
                    <div class="dropdown">
        ...
        function applyFilters(data) {
            ...
            return data.filter(row => {
                return Object.entries(filters).every(([key, filter]) => {
                    ...
                    let rowVal = row[key];
                    let inputVal = filter.val1;

                    // Handle date fields
                    if (col.type === 'date') {
                        if (!rowVal || !inputVal) return false;

                        const rowDate = new Date(rowVal);
                        const filterDate = new Date(inputVal);
                        if (isNaN(rowDate) || isNaN(filterDate)) return false;

                        switch (filter.op) {
                            case 'eq': return rowDate.toDateString() === filterDate.toDateString();
                            case 'neq': return rowDate.toDateString() !== filterDate.toDateString();
                            case 'lt': return rowDate < filterDate;
                            case 'gt': return rowDate > filterDate;
                            case 'lte': return rowDate <= filterDate;
                            case 'gte': return rowDate >= filterDate;
                            default: return true;
                        }
                    } else {
                        // Handle text/number fields
                        inputVal = isNaN(inputVal) ? inputVal.toLowerCase() : parseFloat(inputVal);
                        rowVal = typeof rowVal === 'string' ? rowVal.toLowerCase() : rowVal;
        ...
        $(document).ready(() => {
            ...
            $(document).on('click', '.apply-filter', function () {
                ...
                if (inputVal !== '') {
                    filters[key] = { op: operator, val1: inputVal, val2: null };
                } else {
                    delete filters[key];
                }
                page = 1;
